...
from quant.strategy_gates.gate_models import FAIL, PASS, WARNING, GateConfig, GateResult, final_status
from quant.strategy_gates.gate_registry import GateRegistry
from quant.strategy_gates.gate_report import StrategyGateReportStore
from quant.strategy_gates.gate_rules import fail_gate, num, pass_gate, ratio, reject_gate, warn_gate
...
class StrategyGateRunner:
    ...
    def run(
        ...
        gate_results = [
            self._schema_gate(validation, config),
            self._data_quality_gate(symbols, definition, config),
            self._factor_history_gate(definition, config),
            self._walk_forward_gate(definition, config),
            self._regime_gate(definition, config),
            self._trading_simulation_gate(definition, strategy_run_report, config),
            self._complexity_gate(definition, config),
        ]
        ...
        if write_report:
            ...
            report["generated_reports"] = [report["report_path"]]
    ...
    def _schema_gate(self, validation: dict[str, Any], config: GateConfig) -> GateResult:
        evidence = {
    ...
    def _data_quality_gate(self, symbols: list[str], definition: StrategyDefinition, config: GateConfig) -> GateResult:
        price_coverage = self.context.data_quality_analyzer.coverage(symbols)
        fundamental_coverage = self.context.fundamental_service.coverage(symbols, parameters={"source": "strategy_gate", "symbols": symbols})
    ...
    def _factor_history_gate(self, definition: StrategyDefinition, config: GateConfig) -> GateResult:
        rows = []
        warnings = []
    ...
    def _walk_forward_gate(self, definition: StrategyDefinition, config: GateConfig) -> GateResult:
        rows = []
        warnings = []
        for factor in self._factor_names(definition):
    ...
    def _regime_gate(self, definition: StrategyDefinition, config: GateConfig) -> GateResult:
    ...
    def _trading_simulation_gate(
        self,
        definition: StrategyDefinition,
        strategy_run_report: dict[str, Any] | None,
        config: GateConfig,
    ) -> GateResult:
        summary = (strategy_run_report or {}).get("trade_sim_summary") or {}
        if not summary:
    ...
    def _complexity_gate(self, definition: StrategyDefinition, config: GateConfig) -> GateResult:
