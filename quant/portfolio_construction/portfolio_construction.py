...
class PortfolioConstructionEngine:
    ...
    def construct(
        ...
        return PortfolioConstructionResult(
            ...
            marginal_risk_contributions=result.marginal_risk_contributions,
            risk_contributions=result.risk_contributions,
            risk_contribution_pct=result.risk_contribution_pct,
            warnings=result.warnings,
            output_targets_path=result.output_targets_path,
    ...
    def _write_targets(self, target_weights: dict[str, float], output_targets: str | Path) -> Path:
        path = Path(output_targets)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(target_weights, indent=2, sort_keys=True), encoding="utf-8")
        return path

    def _write_report(self, result: PortfolioConstructionResult) -> Path:
        self.report_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.report_dir / f"portfolio_construction_{timestamp}.json"
